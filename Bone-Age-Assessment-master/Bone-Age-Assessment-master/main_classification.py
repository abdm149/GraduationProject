...
#return period to 30 after run
checkpoint = keras.callbacks.ModelCheckpoint(filepath='weights/weights.{epoch:02d}-{val_loss:.2f}.hdf5',
                                             save_weights_only=True, period=30)
#return epoch to 60 after run
history = model.fit([x_train, gender_train], y_train, batch_size=batch_size, epochs=1, verbose=1,
                    validation_data=([x_valid, gender_valid], y_valid), callbacks=[checkpoint])
...
model.compile(optimizer=Adam, loss='mean_absolute_error', metrics=['MAE'])
#return epochs to 30 after running
...
weights = model.layers[-1].get_weights()[0]
print(weights.shape)
GAPAttention(model, weights, 'C:\\Users\\d\\Desktop\\University\\Graduation project\\archive\\colab pics\\')
# ShowAttentionV1(base_model,'/raid/chenchao/code/BoneAge/BoneAge/data/train/')
