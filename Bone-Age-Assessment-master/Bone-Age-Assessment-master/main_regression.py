...
print (gender)
Ones = np.ones((x.shape[1],x.shape[2]))
...
for i,layer in enumerate(base_model.layers):
    print (i,layer.name)
input = Input(shape=(480,480,3),name='input1')
...
x = keras.layers.Conv2D(256,kernel_size=(1,1))(output)
...
history=model.fit([x_train,gender_train],y_train,batch_size=batch_size,epochs=50,verbose=1,validation_data=([x_valid,gender_valid],y_valid), callbacks = [checkpoint])
